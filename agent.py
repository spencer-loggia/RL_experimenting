...
class Agent:
    def __init__(self, lr=.001, num_agents=1, partial_env=False):
        self.interface = None
        self.models = [ConvNetwork().float() for i in range(num_agents)]  # .cuda(0)
        self.optimizers = [torch.optim.Adam(self.models[i].parameters(), lr=lr) for i in range(num_agents)]
        self.cur_frame = None
    ...
    def play_game(self, max_explore, completion_ratio, save_exp=True, exploration_decay='linear'):
        ...
        while False in games_over:
            ...
            for i in order:
                ...
                r = 0
                if move == 0:
                    r, next_frame = self.interface.update_board(move_made='l', pid=i)
                elif move == 1:
                    r, next_frame = self.interface.update_board(move_made='r', pid=i)
                elif move == 2:
                    r, next_frame = self.interface.update_board(move_made='u', pid=i)
                elif move == 3:
                    r, next_frame = self.interface.update_board(move_made='d', pid=i)
                else:
                    r, next_frame = self.interface.update_board(pid=i)
    ...
    def load_models(self, iter_to_load, game_mode='snake'):
        for i in range(self.num_agents):
...
if __name__ == "__main__":
    ...
    bob = Agent(lr=.001, num_agents=2)
    bob.load_models(iter_to_load=10000)  # load saved policy to see trained agent play
    # bob.eval(10)
    bob.train(NUM_GAMES=10, max_explore=0, gamma=.9, disp_iter=1, save_iter=1000, game_mode='snake')
